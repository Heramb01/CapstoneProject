...
def make_radar(user_values, cluster_id, data, numeric_features):
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        fillcolor=hex_to_rgba(color, 0.15) if color.startswith('#') else color,
        opacity=0.85
    ))
    fig.add_trace(go.Scatterpolar(
...
with col4:
    weekday_hrs = st.number_input("Weekday Online Time (hrs/day)", min_value=0.0, max_value=8.0,
                                  value=2.5, step=0.1, format="%.1f",
                                  help="Average hours spent online on a typical weekday (0.0 – 8.0)")
with col5:
    weekend_hrs = st.number_input("Weekend Online Time (hrs/day)", min_value=0.0, max_value=10.0,
                                  value=4.0, step=0.1, format="%.1f",
                                  help="Average hours spent online on a typical weekend day (0.0 – 10.0)")
